...
1. Geom Smooth for the main graph
2. geom histogram (line) for the precipitation
3. geom map for the globe
...
weatherplot <- ggplot(weather_clean, aes(x = dateInYear)) +
...
weatherplot <- ggplot(weather_clean, aes(x = dateInYear)) +
  geom_linerange(aes(ymin = RecordLow, ymax = RecordHigh), color = "#ECEBE3", linewidth = 1) +
  geom_linerange(aes(ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA", linewidth = 1) +
  geom_linerange(aes(ymin = Low, ymax = High), color = "#A90248", linewidth = 1)  +   geom_vline(xintercept = 12:365)+
